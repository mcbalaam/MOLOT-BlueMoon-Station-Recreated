...
/datum/controller/subsystem/security_level/proc/set_level(new_level)
	...
	if(new_level >= SEC_LEVEL_GREEN && new_level <= SEC_LEVEL_DELTA && new_level != GLOB.security_level)
		switch(new_level)
			if(SEC_LEVEL_GREEN)
				minor_announce(CONFIG_GET(string/alert_green), "Уровень тревоги был снижен до ЗЕЛЁНЫЙ.")
			...
			if(SEC_LEVEL_BLUE)
				if(GLOB.security_level < SEC_LEVEL_BLUE)
					minor_announce(CONFIG_GET(string/alert_blue_upto), "Уровень тревоги был поднят до СИНЕГО!",1)
					if(SSshuttle.emergency.mode == SHUTTLE_CALL || SSshuttle.emergency.mode == SHUTTLE_RECALL)
						SSshuttle.emergency.modTimer(0.6)
				else
					minor_announce(CONFIG_GET(string/alert_blue_downto), "Уровень тревоги был снижен до СИНЕГО.")
					if(SSshuttle.emergency.mode == SHUTTLE_CALL || SSshuttle.emergency.mode == SHUTTLE_RECALL)
						if(GLOB.security_level >= SEC_LEVEL_RED)
...
/proc/epsilon_process()
	minor_announce(CONFIG_GET(string/alert_epsilon), "ТРЕВОГА: КОД - ЭПСИЛОН!")
...
/proc/lambda_process()
	minor_announce(CONFIG_GET(string/alert_lambda), "ТРЕВОГА: КОД - ЛЯМБДА!")
